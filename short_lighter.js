(function() {
    ...
    const config = {
        ...
        // 时间控制
        sleepDuration: 1650000, // 开空仓后休眠时间(毫秒)
    ...
    async function executeTradingCycle() {
        ...
        try {
            // 整点开空仓
            console.log(`[${currentTime}] 第${iteration}次执行 - 开空仓`);
            await openPositionWithRetry('short');
            
            // 休眠
            console.log(`[${currentTime}] 开空仓成功，开始休眠${config.sleepDuration/1000}秒...`);
            await sleep(config.sleepDuration);
            console.log(`[${formatTime(new Date())}] 休眠结束`);

            // 休眠结束后开多仓
            console.log(`[${formatTime(new Date())}] 开多仓`);
    ...
    async function mainLoop() {
        ...
        console.log('策略: 整点开空仓 → 休眠100秒 → 开多仓');
    ...
    console.log('策略: 每分钟整点开空仓，休眠100秒后开多仓');
    console.log('控制命令:');
